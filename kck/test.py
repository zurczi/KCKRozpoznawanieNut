...
def skaner(img, x_pocz, yn):
    ...
    for x in range(x_pocz, img.shape[1] - 1, 1):
        ...
        for i in range(len(yn)):
            wsp_linii[i * 2] = min(yn[i] - delta_linii, 199)
            wsp_linii[(i * 2) + 1] = min(yn[i] + delta_linii, 199)

        jest_czarny_akt = False
        #wykrywanie zmiany koloru
        for i in range(len(wsp_linii)):

            if img[wsp_linii[i], x] != img[wsp_linii[i], x + 1]:
        ...
        if poczatek == 0:
            if od_zmiany >= int(rozmiar_nuty / 6) and sum(wykryte_linie) == 2 and delta >= 6 and delta <= 10  and not jest_czarny_pz and not jest_czarny_akt and pierwsza_zmiana == 2:
                print("cala nuta {} delta {}".format(piecio.nazwa_dzwieku(wykryte_linie_srodek), delta))
                img[:, x-1] = 128
                reset = 1
                ...
                img[:, x-1] = 128
                reset = 1
            elif od_zmiany >= int(rozmiar_nuty / 3) and sum(wykryte_linie) >= 4 and sum(wykryte_linie) <= 5 and delta >= 11 and delta <= 20 and not jest_czarny_akt:
...
if __name__ == '__main__':
    ...
    for i, img in enumerate(img_table):
        ...
        delta_linii = piecio.srednia_delta_linii(yn)
        x_pocz = 8
        odl_skanowania = 7
